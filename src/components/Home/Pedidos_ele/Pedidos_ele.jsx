...
function ListadoAccesorios() {
    ...
    const handleAnadir = (accesorio) => {
        ...
        const id = accesorio.id_accesorio || accesorio.serial;
        const yaExiste = accesoriosSeleccionados.some(item => (item.id_accesorio || item.serial) === id);

        if (!yaExiste) {
            setAccesoriosSeleccionados([...accesoriosSeleccionados, accesorio]);
        }
    };
    ...
    const handleQuitar = (idParaQuitar) => {
        setAccesoriosSeleccionados(accesoriosSeleccionados.filter(item => 
            (item.id_accesorio || item.serial) !== idParaQuitar
        ));
    };

    //Logica de useEffect para obtener los datos
    useEffect(() => {
...
{/*Modal para mostrar los equipos seleccionados  */}
<Modal show={mostrarModal} onHide={handleCerrarModal}>
<Modal.Header className='modal_header_Acce' closeButton>
<Modal.Title>Accesorios Seleccionados ({accesoriosSeleccionados.length})</Modal.Title>
</Modal.Header>
<Modal.Body>
{accesoriosSeleccionados.length === 0 ? (
...
<ul className="lista-seleccionados">
  {accesoriosSeleccionados.map(item => {
    const idItem = item.id_accesorio || item.serial;
  return (
    <li key={idItem}>
...
<Modal.Footer>
  <Button className='btn_cerrar_mdl_ele' variant="secondary" onClick={handleCerrarModal}> Cerrar </Button>
    <Button className='Btn_add_acce_modal' variant="primary" onClick={() => {
      if (accesoriosSeleccionados.length === 0) {
        alert('No has seleccionado ningún accesorio.');
      } else {
