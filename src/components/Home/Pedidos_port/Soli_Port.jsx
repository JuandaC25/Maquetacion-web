    ...
    useEffect(() => {
        const fetchElementos = async () => {
            try {
                ...
                let portatiles = [];
                const subcategoria = categoriaFiltro === "computo" ? "Portatil" : "Portatil de edición";
                portatiles = data.filter((item) =>
                    (item.sub_catg === subcategoria || item.nom_eleme === subcategoria) && item.est === 1
                );
            ...
            {equiposDisponibles.length > 0 ? (
                <Card className="port-ficha-visual">
                    ...
                    <div className="port-ficha-footer">
                        <Button className="port-boton-solicitar" onClick={handleShowModal}>
